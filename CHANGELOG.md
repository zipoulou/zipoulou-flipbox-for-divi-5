...
- **Touch auto-return reverted** — the 3.5 s auto-return on touch fallback was removed after real-world testing: visitors read the back side as "flipping by itself" and the return transition competed with the initial flip, producing a jerky motion. Touch now uses plain toggle semantics (tap flips, tap-again flips back). The `autoReturnMs` parameter of `initClick` stays in place for future opt-in use.
- **Mouseenter wobble gated on trigger** — the "replay wobble on every hover" feature (introduced earlier in this release) now skips hover-triggered flipboxes. On those the CSS `:hover` flip already fully replaces transform, so layering a 500 ms wobble keyframe on top caused a visible snap at animation-end when the transition hadn't quite reached its target. Click and auto triggers keep the hover wobble as their primary hover feedback; hover trigger relies on the flip itself.
